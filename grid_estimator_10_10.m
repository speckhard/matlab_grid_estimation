...
% Take the real/imaginary compoenents of the data.
tic % measure timing performance of this section of code
measurement_clean = @measurement_cleaner;
v_clean_matrix = measurement_clean(v_vec, theta_vec);
disp('time required to clean data')
toc

% Find the entropy of each node, H(i).
tic
find_single_node_entropy = @single_node_entropy;
single_node_entropy_vec = find_single_node_entropy(v_clean_matrix);
disp('time required to find entropy')
toc

% Find the joint entropy of pairs of different nodes, H(i,j).
tic 
find_joint_entropy = @joint_entropy;
joint_entropy_matrix = find_joint_entropy(v_clean_matrix);
...
text(h.XData(i),h.YData(i),num2str(i),'fontsize',10);
end
...
for i = 1:(number_of_buses-1)
    ...
    if (ismember(valid_branch_node_pairs(i,:), ...
            mpc_base.branch(:,1:2) - ...
            ones(number_of_buses,2),'rows'))
        sdr_counter = sdr_counter +1;
    elseif (ismember([valid_branch_node_pairs(i,2), ... 
            valid_branch_node_pairs(i,1)], mpc_base.branch(:,1:2)-...
            ones(number_of_buses,2),'rows'))
        sdr_counter = sdr_counter +1;
    end
