...
% Gaussian data
sdr_mat_g = SG1results.mean_sdr_mat(1,3:(end),1);
std_sdr_mat_g = SG1results.std_sdr_mat(1,3:(end),1);

% JVHW data
sdr_mat_j = SG1results.mean_sdr_mat(1,3:(end),2);
std_sdr_mat_j = SG1results.std_sdr_mat(1,3:(end),2);

% Discrete data
sdr_mat_d = SG1results.mean_sdr_mat(1,3:(end),3);
std_sdr_mat_d = SG1results.std_sdr_mat(1,3:(end),3);
...
%% SG1 -solar plots
% Gaussian data
sdr_mat_g = SG1solarresults.mean_sdr_mat(1,3:(end),1);
std_sdr_mat_g = SG1solarresults.std_sdr_mat(1,3:(end),1);

% JVHW data
sdr_mat_j = SG1solarresults.mean_sdr_mat(1,3:(end),2);
std_sdr_mat_j = SG1solarresults.std_sdr_mat(1,3:(end),2);

% Discrete data
sdr_mat_d = SG1solarresults.mean_sdr_mat(1,3:(end),3);
std_sdr_mat_d = SG1solarresults.std_sdr_mat(1,3:(end),3);
...
linkaxes([ax4,ax3],'xy')
linkaxes([ax5,ax6],'xy')
axis([ax3 ax4],[0 400 90 97])
axis([ax1 ax2],[0 400 90 97])
axis([ax5 ax6],[0 400 90 97])
